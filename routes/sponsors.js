var config = require( '../config' )
    , dao = require( '../models/dao' )( 'sponsor' ),
      fs = require( 'fs' );

exports.findById = function( req, res ) {
    dao.query( 'all', {key: [req.params.id], reduce: false}, function( err, sponsor ) {
        if( err ) {
            res.send( 404, 'We could not locate that sponsor' );
        } else {
            res.send( JSON.stringify( sponsor ) );
        }
    } );
};
...
exports.generate = function( req, res ) {
    dao.list( req.cookies['AuthSession'], function( err, list ) {
        if( err ) {
            ...
            fs.writeFile( __dirname + '/../public/sponsors.json', JSON.stringify( list, null, 4 ), function( err ) {
                if( err ) {
                    res.send( 401, JSON.stringify( { error: true } ) );
                } else {
